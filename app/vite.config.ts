...
export default defineConfig(({ mode, command }) => {
  ...
  return {
    ...
    // Build-only: in `vite dev` the SSR server runs on Node where bare imports
    // resolve normally, and forcing CJS deps (react) through the dev module
    // runner crashes with "module is not defined".
    ssr: {
